#!/usr/bin/env python3
"""Run ClusterRT-LRC initial-layout normal I/O and single-block recovery tests."""
...
def build_runs(manifest: Dict[str, Any], tests: Optional[Sequence[str]] = None,
    ...
    for test in selected:
        for encoding in manifest["encoding_parameters"]:
            identity = json.dumps(
                ("ClusterRT_LRC", test, encoding["name"], encoding["k"],
...
def render_xml(source: bytes, run: Dict[str, Any]) -> bytes:
    ...
    replacements = {
        ...
        "r": run["g"], "z": run["l"], "CodeType": "ClusterRT_LRC",
    }
