#!/usr/bin/env python3
"""Plan and run the DdlRT-LRC overnight experiment matrix."""
...
DEFAULT_OUTPUT = REPO_ROOT.parent / (REPO_ROOT.name + "-experiment-results")
CSV_FIELDS = [
    "run_id", "status", "failure_reason", "attempts", "experiment_labels",
...
def _canonical_key(config: Dict[str, Any]) -> Tuple[Any, ...]:
    return (config["encoding"], config["k"], config["l"], config["g"],
...
def build_configurations(manifest: Dict[str, Any], experiments: Optional[Set[str]] = None,
                         stripes_override: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    for encoding in manifest["encoding_parameters"]:
        base = {
            "encoding": encoding["name"], "k": encoding["k"],
...
def render_xml(source: bytes, block_size: int, k: int, r: int, z: int) -> bytes:
    root = ET.fromstring(source)
    replacements = {"BlockSize": block_size, "k": k, "r": r, "z": z}
...
def update_xml_file(path: Path, block_size: int, k: int, r: int, z: int) -> bytes:
    original = path.read_bytes()
    atomic_write(path, render_xml(original, block_size, k, r, z))
    return original
        ...
        with log_path.open("w", encoding="utf-8", buffering=1) as log:
            ...
            def consume_output(output: str) -> None:
                ...
                while True:
                    ...
                    payload = answer + "\n"
                    if answer == "Y":
                        # Client::start_merge reads the round number after the Y/N answer.
                        payload += str(merge_round) + "\n"
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    configurations = build_configurations(manifest, selected, args.stripes)
    ...
    for config in configurations:
        print("{config_id} labels={labels} encoding={encoding} k/l/g={k}/{l}/{g} "
