...
bool CoordinatorImpl::recovery_one_block(int stripe_id, int failed_block_id) {
  ...
  // DdlRT_LRC, SRS, and ERS data repair use the same execution path:
  // select the other data blocks in the local group plus its local parity,
  // group them by physical rack, and send one aggregate per helper rack.
  const bool aggregated_local_data_repair =
      (code_type == "DdlRT_LRC" || code_type == "SRS" ||
       code_type == "ERS") &&
      failed_block_id < t_stripe.k;
  if (aggregated_local_data_repair) {
    ...
    for (const auto &helper : helper_groups) {
      threads.emplace_back([this, helper, dest_proxy_ip, dest_proxy_port,
                            failed_block_id, failed, code_type, &repair_ok]() {
        ...
        if (!status.ok()) {
          ...
          std::cout << "[Coordinator] " << code_type << " helper rack "
    ...
    threads.emplace_back([this, &source_blocks_by_cluster, &repair_ok,
                          ...
                          failed_block_id, failed, code_type,
      ...
      if (!status.ok()) {
        repair_ok.store(false);
        std::cout << "[Coordinator] " << code_type
                  << " destination rack failed: " << status.error_message()
                  << std::endl;
      }
    });
    ...
    if (repair_ok.load()) {
      std::cout << "[Coordinator] " << code_type
                << " aggregated recovery of " << stripe_id << "_"
                << failed_block_id << " success!" << std::endl;
      return true;
    }
    std::cout << "[Coordinator] " << code_type
