...
bool CoordinatorImpl::recovery_one_block(int stripe_id, int failed_block_id) {
  ...
  // DdlRT_LRC data repair uses the surviving data in the same local group
  ...
  if (code_type == "DdlRT_LRC" && failed_block_id < t_stripe.k) {
    ...
    for (const auto &helper : helper_groups) {
      threads.emplace_back([this, helper, dest_proxy_ip, dest_proxy_port,
        ...
        if (!status.ok()) {
          ...
          std::cout << "[Coordinator] DdlRT_LRC helper rack "
    ...
    threads.emplace_back([this, &source_blocks_by_cluster, &repair_ok,
      ...
      if (!status.ok()) {
        repair_ok.store(false);
        std::cout << "[Coordinator] DdlRT_LRC destination rack failed: "
                  << status.error_message() << std::endl;
      }
    });
    ...
    if (repair_ok.load()) {
      std::cout << "[Coordinator] DdlRT_LRC aggregated recovery of "
